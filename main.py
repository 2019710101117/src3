...
class MyFrame(wx.Frame):
    def __init__(self, parent, id):
        ...
        # 创建文本，左对齐        
        self.title = wx.StaticText(panel,
                                   label="1、idkp1-10.txt    2、sdkp1-10.txt    3、udkp1-10.txt    4、wdkp1-10.txt\n")
...
class MyFrame3(wx.Frame):
    def __init__(self, parent, id):
        ...
        for j in range(0, d):
            height = height + 20
            wx.StaticText(self.panel, label='%8d  %8d  %8d' % (result[j][0], result[j][1], result[j][2]),
                          pos=(100, height))
        # 价值：重量比排序
        global third
    ...
    def CreateDataBase(self):
        ...
        for j in range(0, d):
            cursor.execute(
                'insert into user (num,profit,weight) values ("%d","%d","%d")' % (j, profit[i][j], weight[i][j]))
